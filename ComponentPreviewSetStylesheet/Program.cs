            ...
            try
            {
                ...
                if (jsonObject.TryGetValue("Blocks", out JToken? blocksToken) && blocksToken is JArray blocksArray)
                {
                    ...
                    foreach (var block in blocksArray)
                    {
                        ...
                        if (block["stylesheet"] != null)
                        {
                            block["stylesheet"] = "~/app/assets/general-dist.css";
                            modified = true;
                        }
...
Console.WriteLine($"Processing complete. Stylesheet has now been applied to: {fileModifiedCount} files.");